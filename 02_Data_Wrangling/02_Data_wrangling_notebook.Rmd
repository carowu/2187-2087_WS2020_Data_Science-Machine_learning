...
Please save this notebook under a new file name. The course github-repo will be updated regularly and your edits might result in merge conflicts if you pull the most recent commit.
...
For this, create a project via the R-Studio GUI:
...
For more complicated project structures with multiple directories containing code,  Rmarkdown files and input data, building relative paths that work can become cumbersome. For this the package "here" can be used to construct file paths. The package will use a set of heuristics to create an absolute path relative to your R-project structure. The resulting path will work on Windows and Mac Os.
...
If you are using a version control software like Github you have to make sure that the "data" folder is included in the gitignore file because right out of the box Github won't accept big files and Github is for code not for data.


## Big to medium sized

If you are collaborating on a medium to big project using a version control system like GitHub you should separate the code of the R-project from the input data. This is also good practice if the data is confidential and must not leave the computers or the premises of your workplace. In a bigger workplace your data might also be stored in a database or on a file server. If there are passwords needed to access these data it is **very** important to store these secrets locally and not on github. 
...
Fortunatlely the underlying data-format has been standardized by Microsoft a few years ago. It is not quite easy to generate and read in excel files.
...
This is why the tidyverse way is to chain data wrangling commands together with the pipe operator: %>%
...
cleaned_remittances_small <- remittances %>%
  ...
  #Filter cols for Austria and Germany
  select(c("name_o","Austria","Germany")) 

cleaned_remittances_small
```

The filter command can be used with a bunch of other boolean operations. Heres a handy graphic for the synthax, taken from [R for datascience](https://r4ds.had.co.nz/transform.html). Please use the respective chapter as refervariables
...
This is A-OK. Nobody expects you to memorize synthax although you will remember it on your own at some point (or not). But you do have to be able to solve problems using the help and google. For this it is necessary to understand the concepts behind these operations and you might be asked about some of these and other concepts in the finals (**hint**).
...
wdi <- read_csv(here("data","Worldbank","World_Development_Indicators","WDIData.csv"))
...
mean_temps %>%
  select(month, average_temp, estimate, difference)
...
Remember the "tidy" remittances matrix? This object can be interpreted as a joint denisty of origin ($o$) and destination ($d$) tuples and their respective remittances value $f_{o,d}(o,d) = remit_{o,d}$. You can think of it it as a "lookup" function in the table if you like.
